...
class PHPWebSocket
{
                ...
                foreach ($changed as $clientID => $socket) {
                    if ($clientID != 0) {
                        ...
                        if ($client !== false) {
                            ...
                            if ($result !== false && $this->wsClientCount < self::WS_MAX_CLIENTS
    ...
    function wsAddClient($socket, $clientIP)
    {
        ...
        $this->wsClients[$clientID] = array(
            $socket, '', self::WS_READY_STATE_CONNECTING, time(), false, 0, $clientIP, false, 0, '', 0, 0
        );
    ...
    function wsRemoveClient($clientID)
    {
        ...
        if (array_key_exists('close', $this->wsOnEvents)) {
            foreach ($this->wsOnEvents['close'] as $func) {
                call_user_func($func, $clientID, $closeStatus);
            }
        }
        ...
        if ($this->wsClients[$clientID][8] > 1) {
            ...
            if ($this->wsClients[$clientID][7] !== false) {
                ...
                if (!$controlFrame) {
                    ...
                    if ($newMessagePayloadLength > self::WS_MAX_MESSAGE_PAYLOAD_RECV
                        || $newMessagePayloadLength > 2147483647
                    ) {
                        $this->wsSendClientClose($clientID, self::WS_STATUS_MESSAGE_TOO_BIG);
                        return false;
                    }
    ...
    function wsProcessClientFrame($clientID)
    {
        ...
        $maskKey = $array['a'];
        $maskKey = array(
            $maskKey >> 24, ($maskKey >> 16) & 255, ($maskKey >> 8) & 255, $maskKey & 255
        );
        $seek += 4;
        ...
        if ($fin == self::WS_FIN) {
            ...
            if ($opcode != self::WS_OPCODE_CONTINUATION) {
                ...
                // process the message
                $result = $this->wsProcessClientMessage($clientID, $this->wsClients[$clientID][10],
                    $this->wsClients[$clientID][1], $this->wsClients[$clientID][11]
                );
        ...
        if ($opcode == self::WS_OPCODE_PING) {
            ...
            if (array_key_exists('message', $this->wsOnEvents)) {
                foreach ($this->wsOnEvents['message'] as $func) {
                    call_user_func($func, $clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
                }
            }
        } else {
            // unknown opcode
    ...
    function wsSendClientMessage($clientID, $opcode, $message)
    {
        ...
        for ($i = 0; $i < $frameCount; $i++) {
            ...
            // set frame bytes
            $buffer = pack('n', (($fin | $opcode) << 8) | $payloadLength) . $payloadLengthExtended . substr($message,
    ...
    function wsSendJson($clientID, $data)
    {
        return $this->wsSendClientMessage($clientID, self::WS_OPCODE_TEXT, json_encode($data));
    }
